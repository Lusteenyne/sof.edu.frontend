...
const TeacherAssignment = () => {
  ...
  useEffect(() => {
    const fetchCourses = async () => {
      try {
        const res = await fetch('http://localhost:5003/teacher/courses', {
  ...
  const fetchAssignments = async () => {
    try {
      const res = await fetch(`http://localhost:5003/teacher/course/${selectedCourseId}/assignments`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
      setAssignments(data || []);
  ...
  const fetchSubmissions = async (assignmentId) => {
    try {
      const res = await fetch(`http://localhost:5003/teacher/assignments/${assignmentId}/submissions`, {
        headers: { Authorization: `Bearer ${token}` },
      });
      const data = await res.json();
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const url = editId
        ? `http://localhost:5003/teacher/assignments/${editId}`
        : `http://localhost:5003/teacher/course/${selectedCourseId}/give-assignments`;
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const res = await fetch(`http://localhost:5003/teacher/assignments/${id}`, {
        method: 'DELETE',
        headers: { Authorization: `Bearer ${token}` },
      });
      if (res.ok) {
  ...
  const handleGradeSubmit = async (submissionId, score) => {
    ...
    try {
      const res = await fetch(`http://localhost:5003/teacher/assignments/submission/${submissionId}/grade`, {
        method: 'POST',
          ...
          <ul className="assignment-list">
            {assignments.map((a) => (
              <li key={a._id}>
                ...
                {a.fileUrls?.length > 0 && (
                  <div>
                    <a href={`http://localhost:5003${a.fileUrls[0]}`} target="_blank" rel="noopener noreferrer" download>
                      Download File
                    </a>
                  </div>
                )}
                      ...
                      submissions.map((s) => (
                        <div key={s._id} className="submission-card">
                          ...
                          {s.fileUrls?.map((url, idx) => (
                            <div key={idx}>
                              <a href={`http://localhost:5003${url}`} target="_blank" rel="noopener noreferrer" download>
                                View / Download File
                              </a>
                            </div>
                          ))}

...
const StudentDetails = ({
  ...
  const handleSave = async () => {
    try {
      ...
      await axios.patch(
        `http://localhost:5003/admin/students/${student._id}/update`,
        editData,
...
const SuperAdminStudents = () => {
  ...
  const fetchCourses = useCallback(async () => {
    try {
      const token = localStorage.getItem("admin_token");
      const res = await axios.get("http://localhost:5003/admin/courses", {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const loadStudentResults = useCallback(async (studentId) => {
    try {
      const token = localStorage.getItem("admin_token");
      const res = await axios.get(`http://localhost:5003/admin/students/${studentId}/results`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      await axios.delete(`http://localhost:5003/admin/students/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const approveCourse = async (studentId, courseId) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.patch(
        `http://localhost:5003/admin/students/${studentId}/courses/${courseId}/approve`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const rejectCourse = async (studentId, courseId) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.patch(
        `http://localhost:5003/admin/students/${studentId}/courses/${courseId}/reject`,
        {},
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const approveGrade = async (studentId, grade) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.patch(
        `http://localhost:5003/admin/students/${studentId}/results/approve`,
  ...
  const submitToTeacher = async (studentId, grade) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.post(
        `http://localhost:5003/admin/students/${studentId}/results/submit-to-teacher`,
        { grade },
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const updateStudentInfo = async (studentId, data) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.patch(
        `http://localhost:5003/admin/students/${studentId}/update-info`,
        data,
        { headers: { Authorization: `Bearer ${token}` } }
      );

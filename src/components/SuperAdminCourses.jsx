...
const SuperAdminCourses = () => {
  ...
  const handleAddCourse = async () => {
    ...
    try {
      await axios.post("http://localhost:5003/admin/courses", newCourse, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleDeleteCourse = async (id) => {
    ...
    try {
      await axios.delete(`http://localhost:5003/admin/courses/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
  ...
  const handleSaveEdit = async () => {
    try {
      await axios.patch(
        `http://localhost:5003/admin/courses/${editingId}`,
        editedCourse,
        {
          headers: { Authorization: `Bearer ${token}` },
        }

...
const SuperAdminProfile = () => {
  ...
  useEffect(() => {
    ...
    const fetchProfile = async () => {
      setLoading(true);
      try {
        const res = await fetch('https://sof-edu-backend.onrender.com/admin/profile', {
  ...
  const handlePhotoUpload = async () => {
    ...
    try {
      const res = await fetch('https://sof-edu-backend.onrender.com/admin/upload-profile-photo', {
        method: 'POST',
        headers: { Authorization: `Bearer ${token}` },
        body: formData,
      });

...
const SuperAdminTeachers = () => {
  ...
  const handleAction = async (id, action) => {
    try {
      const token = localStorage.getItem("admin_token");
      await axios.patch(
        `http://localhost:5003/admin/teachers/${id}/${action}`,
        null,
        { headers: { Authorization: `Bearer ${token}` } }
      );
  ...
  const handleDelete = async (id) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      await axios.delete(`http://localhost:5003/admin/teachers/${id}`, {
        headers: { Authorization: `Bearer ${token}` },
      });
...
const assignCourses = async (teacherId, index) => {
  ...
  try {
    const token = localStorage.getItem("admin_token");
    await axios.patch(
      `http://localhost:5003/admin/teachers/${teacherId}/assign-courses`,
  ...
  const handleRemoveCourse = async (teacherId, courseId, index) => {
    ...
    try {
      const token = localStorage.getItem("admin_token");
      await axios.delete(
        `http://localhost:5003/admin/teachers/${teacherId}/remove-course/${courseId}`,
        { headers: { Authorization: `Bearer ${token}` } }
      );

...
const TeacherClasses = () => {
  ...
  const fetchSubmittedResults = async (courseId) => {
    try {
      const res = await fetch(
        `http://localhost:5003/teacher/course/${courseId}/submitted-results`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const fetchStudents = async (courseId) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5003/teacher/course/${courseId}/students`,
        {
          headers: { Authorization: `Bearer ${token}` },
        }
      );
  ...
  const handleSubmitGrades = async () => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5003/teacher/course/${selectedCourse._id}/submit-results`,
        {
          method: 'POST',
          headers: {
